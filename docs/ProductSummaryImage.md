...
| `hoverImageCriteria` | `enum` | The criteria that will be used to match the hover image in the product images list from the admin's Catalog. The `label` option will match against the `imageLabel` fild from the admin's Catalog, and the `index` will just match the image in that index in the list. | `label` |
...
| `hoverImageIndex` | `Number` | The index that matches the image in the product images list from the admin's Catalog. | `undefined` |
