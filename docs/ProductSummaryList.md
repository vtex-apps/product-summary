...
| `hideUnavailableItems` | `boolean`                              | Hides unavailable items. When omitted, the effective value is `false`. If the Delivery Promise feature is enabled, an omitted value is treated as `true` instead. | `undefined`                  |
